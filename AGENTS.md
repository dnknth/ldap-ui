...
- **Version bump**: `__version__` (`backend/ldap_ui/__init__.py`) is embedded in `info.version`, so bumping needs the fixture updated too — and conversely, any change that touches `tests/resources/openapi.json` (route or model edits) requires a version bump to go with it. Ask the user to determine the next version; never pick one yourself.
...
- Never commit (or amend) without asking the user first.
